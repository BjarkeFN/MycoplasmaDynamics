...
idx_range = np.arange(1, N + Npred - Nnpi + 1)
noisewhich = np.ceil(idx_range / 2.0).astype(int)
...
stan_data = {
    ...
    "noisewhich": noisewhich.tolist() if len(noisewhich) > 0 else [],
    "week": week_stan,
...
stan_file = "stan_stochastic_2025.stan"

sir_model = CmdStanModel(stan_file=stan_file)

n_chains = 4
inits = [{'S0': 0.6, 'logx_I0': -2.5, 'beta0': 0.7, 'dbeta': 0.15, 'betaphase': 3.6, 'sigma_obs': 0.10, 'logrho': -1.7, 'delta': 0.002}] * n_chains  
...
output_dir = "stan_output/stochastic_2025/"
os.makedirs(output_dir, exist_ok=True)

fit.save_csvfiles(dir=output_dir)

summary_df = fit.summary()
summary_path = os.path.join(output_dir, "stanfit_sirs_stoch_posfrac_inc_summary.csv")
